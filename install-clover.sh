...
Choice=\$(zenity --width 750 --height 400 --list --radiolist --multiple \
	...
	FALSE Logo "Use random logos or original logo when booting to Windows."\\
...
Timeout_Choice=\$(zenity --width 500 --height 275 --list --radiolist --multiple \
	...
	FALSE 1sec "Set the default timeout to 1sec."\\
	...
	elif [ "\$Timeout_Choice" == "1sec" ]
	then
		# change the Default Timeout to 1sec in config,plist 
		echo \$PASSWORD | sudo -S sed -i '/<key>Timeout<\\/key>/!b;n;c\\\t\\t<integer>1<\\/integer>' /esp/efi/clover/config.plist
		zenity --warning --title "Clover Toolbox" --text "Default timeout is now set to 1sec!" --width 400 --height 75

	elif [ "\$Timeout_Choice" == "5secs" ]
	then
		# change the Default Timeout to 5secs in config,plist 
		echo \$PASSWORD | sudo -S sed -i '/<key>Timeout<\\/key>/!b;n;c\\\t\\t<integer>5<\\/integer>' /esp/efi/clover/config.plist
	...
	then
		...
		echo \$PASSWORD | sudo -S sed -i '/<key>Timeout<\\/key>/!b;n;c\\\t\\t<integer>10<\\/integer>' /esp/efi/clover/config.plist
	...
	then
		...
		echo \$PASSWORD | sudo -S sed -i '/<key>Timeout<\\/key>/!b;n;c\\\t\\t<integer>15<\\/integer>' /esp/efi/clover/config.plist
...
Boot_Choice=\$(zenity --width 550 --height 250 --list --radiolist --multiple --title "Clover Toolbox" --column "Select One" \\
	...
	then
		...
		# test if Windows is installed on the internal SSD
		echo \$PASSWORD | sudo -S ls /esp/efi | grep Microsoft

		if [ \$? -eq 0 ]
		then
			# Windows is installed on the internal SSD - use the custom splash screen!
			echo \$PASSWORD | sudo -S sed -i '/<key>DefaultLoader<\\/key>/!b;n;c\\\t\\t<string>\\\EFI\\\HackBGRT\\\HackBGRT\\.efi<\\/string>' /esp/efi/clover/config.plist
			echo \$PASSWORD | sudo -S sed -i '/<key>DefaultVolume<\\/key>/!b;n;c\\\t\\t<string>esp<\\/string>' /esp/efi/clover/config.plist
			echo \$PASSWORD | sudo -S sed -i 's/\\\EFI\\\MICROSOFT\\\bootmgfw\.efi/\\\EFI\\\HackBGRT\\\HackBGRT\.efi/g' /esp/efi/clover/config.plist
		else
			# Windows is not installed on the internal SSD - do not use the custom splash screen!
			...
			echo \$PASSWORD | sudo -S sed -i '/<key>DefaultVolume<\\/key>/!b;n;c\\\t\\t<string>esp<\\/string>' /esp/efi/clover/config.plist
			echo \$PASSWORD | sudo -S sed -i 's/\\\EFI\\\HackBGRT\\\HackBGRT\.efi/\\\EFI\\\MICROSOFT\\\bootmgfw\.efi/g' /esp/efi/clover/config.plist
		fi
...
then
Logo_Choice=\$(zenity --width 750 --height 250 --list --radiolist --multiple --title "Clover Toolbox" --column "Select One" \\
	--column "Option" --column="Description - Read this carefully!"\\
	FALSE NewLogo "Use random logo when booting to Windows (internal SSD only)."\\
	FALSE OldLogo "Use original logo when booting to Windows."\\
	TRUE EXIT "***** Exit the Clover Toolbox *****")

	if [ \$? -eq 1 ] || [ "\$Logo_Choice" == "EXIT" ]
	then
		echo User pressed CANCEL / EXIT.
		exit
	...
	then
		...
		echo \$PASSWORD | sudo cp /esp/efi/HackBGRT/config_random.txt /esp/efi/HackBGRT/config.txt
	...
	then
		...
		echo \$PASSWORD | sudo cp /esp/efi/HackBGRT/config_orig.txt /esp/efi/HackBGRT/config.txt
...
then
	...
	echo \$PASSWORD | sudo -S rm -rf /esp/efi/HackBGRT
